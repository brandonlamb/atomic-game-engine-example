...
declare module Atomic {
  ...
  export var SOUND_MASTER:string;
  export var SOUND_EFFECT:string;
  export var SOUND_AMBIENT:string;
  export var SOUND_VOICE:string;
  export var SOUND_MUSIC:string;
  export var game:any;
...
//----------------------------------------------------
  ...
  export class CustomGeometry extends Drawable {
    ...
    defineGeometry(index:number, type:PrimitiveType, numVertices:number, hasNormals:boolean,
                   hasColors:boolean, hasTexCoords:boolean, hasTangents:boolean):void;
  ...
  export class DecalSet extends Drawable {
    ...
    addDecal(target:Drawable, worldPosition:Vector3, worldRotation:Quaternion, size:number,
             aspectRatio:number, depth:number, topLeftUV:Vector2, bottomRightUV:Vector2,
             timeToLive?:number, normalCutoff?:number, subGeometry?:number):boolean;
...
//----------------------------------------------------
  ...
  export class Audio extends AObject {
    ...
    setMode(bufferLengthMSec:number, mixRate:number, stereo:boolean,
...
//----------------------------------------------------
  ...
  export class DebugRenderer extends Component {
    ...
    /**  Add a cylinder */
    addCylinder(position:Vector3, radius:number, height:number, color:Color,
                depthTest?:boolean):void;
  ...
  export class Graphics extends AObject {
    ...
    drawInstanced(type:PrimitiveType, indexStart:number, indexCount:number, minVertex:number,
                  vertexCount:number, instanceCount:number):void;
    ...
    /**  Set stencil test. */
    setStencilTest(enable:boolean, mode?:CompareMode, pass?:StencilOp, fail?:StencilOp,
                   zFail?:StencilOp, stencilRef?:number, compareMask?:number,
  ...
  export class Light extends Drawable {
    ...
    setShadowCascade(split1:number, split2:number, split3:number, split4:number, fadeStart:number,
                     biasAutoAdjust?:number);
  ...
  export class Material extends Resource {
    ...
    setShaderParameterAnimation(name:string, animation:ValueAnimation, wrapMode?:WrapMode,
                                speed?:number):void;
  ...
  export class Renderer extends AObject {
    ...
    getScreenBuffer(width:number, height:number, format:number, cubemap:boolean, filtered:boolean,
                    srgb:boolean, persistentKey?:number):Texture;
...
//----------------------------------------------------
  ...
  export class Input extends AObject {
     ...
     *  MM_ABSOLUTE is the default behaviour, allowing the toggling of operating system cursor
     * visibility and allowing the cursor to escape the window when visible. When the operating
     * system cursor is invisible in absolute mouse mode, the mouse is confined to the window. If
     * the operating system and UI cursors are both invisible, interaction with the Urho UI will be
     * limited (eg: drag move / drag end events will not trigger). SetMouseMode(MM_ABSOLUTE) will
     * call SetMouseGrabbed(false).
     *
     *  MM_RELATIVE sets the operating system cursor to invisible and confines the cursor to the
     * window. The operating system cursor cannot be set to be visible in this mode via
     * SetMouseVisible(), however changes are tracked and will be restored when another mouse mode
     * is set. When the virtual cursor is also invisible, UI interaction will still function as
     * normal (eg: drag events will trigger). SetMouseMode(MM_RELATIVE) will call
     * SetMouseGrabbed(true).
     *
     *  MM_WRAP grabs the mouse from the operating system and confines the operating system cursor
     * to the window, wrapping the cursor when it is near the edges. SetMouseMode(MM_WRAP) will
     * call SetMouseGrabbed(true).
     */
...
//----------------------------------------------------
  ...
  export class CollisionShape extends Component {
    ...
    setTriangleMesh(model:Model, lodLevel?:number, scale?:Vector3, position?:Vector3,
                    rotation?:Quaternion):void;

    /**  Set as a triangle mesh from CustomGeometry. */
    setCustomTriangleMesh(custom:CustomGeometry, scale?:Vector3, position?:Vector3,
                          rotation?:Quaternion):void;

    /**  Set as a convex hull from Model. */
    setConvexHull(model:Model, lodLevel?:number, scale?:Vector3, position?:Vector3,
                  rotation?:Quaternion):void;

    /**  Set as a convex hull from CustomGeometry. */
    setCustomConvexHull(custom:CustomGeometry, scale?:Vector3, position?:Vector3,
                        rotation?:Quaternion):void;
...
//----------------------------------------------------
  ...
  export class ResourceCache extends AObject {
    ...
    backgroundLoadResource(type:string, name:string, sendEventOnFailure?:boolean,
                           caller?:Resource):boolean;
...
//----------------------------------------------------
  ...
  export class Animatable extends Serializable {
    ...
    setAttributeAnimation(name:string, attributeAnimation:ValueAnimation, wrapMode?:WrapMode,
                          speed?:number):void;
  ...
  export class ObjectAnimation extends Resource {
    ...
    addAttributeAnimation(name:string, attributeAnimation:ValueAnimation, wrapMode?:WrapMode,
                          speed?:number):void;
...
//----------------------------------------------------
  ...
  export class UIMessageWindow extends UIWindow {
    ...
    show(title:string, message:string, settings:UI_MESSAGEWINDOW_SETTINGS, dimmer:boolean,
...
//----------------------------------------------------
  ...
  export class BuildSystem extends Atomic.AObject {
    ...
    buildComplete(platform:PlatformID, buildFolder:string, success?:boolean,
                  buildMessage?:string):void;
...
declare module Editor.Extensions {
  ...
  interface EventDispatcher {
    ...
    /**
     * Subscribe to an event and provide a callback.  This can be used by services to subscribe to
     * custom events
     * @param  {string} eventType
     * @param  {any} callback
     */
  ...
  /**
   * Interface that describes a Resource Editor Factory that will build out the editor for the
  ...
  export interface ResourceEditorBuilder {
     ...
     * @param  tabContainer
     */
    getEditor(resourceFrame:Atomic.UIWidget, resourcePath:string,
              tabContainer:Atomic.UITabContainer):Editor.ResourceEditor;
  }
}
...
declare module Editor.HostExtensions {
  ...
  export interface UIServicesProvider extends Editor.Extensions.ServicesProvider<UIServicesEventListener> {
    ...
    showModalWindow(windowText:string, uifilename:string,
                    handleWidgetEventCB:(ev:Atomic.UIWidgetEvent) => void):Editor.Modal.ExtensionWindow;
    showModalError(windowText:string, message:string);
    showResourceSelection(windowText:string, importerType:string, resourceType:string,
                          callback:(retObject:any, args:any) => void, args?:any);
...
declare module Editor.ClientExtensions {
  ...
  export interface WebViewServicesProvider extends Editor.Extensions.ServicesProvider<WebViewServiceEventListener> {
     ...
     * @return {number|boolean|string}
     */
    getUserPreference(extensionName:string, preferenceName:string,
                      defaultValue?:number | boolean | string):number | boolean | string;
  }
  ...
  export interface HostInterop {
     ...
     * @param  {string} filename name and path of file under the project directory or a fully
     *   qualified file name
     * @return {Promise}
     */
...
//----------------------------------------------------
  ...
  export class WebClient extends Atomic.AObject {
    ...
    sendMouseClickEvent(x:number, y:number, button:number, mouseUp:boolean, modifier:number,
                        clickCount?:number):void;

    /**  Send a mouse press event to the browser */
    sendMousePressEvent(x:number, y:number, button?:number, modifier?:number,
                        clickCount?:number):void;
